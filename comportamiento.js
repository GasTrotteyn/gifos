...
async function CrearEventoBotonesDeHistorial() {
    let botonesHistorial = await document.getElementsByClassName(
        "botonHistorial"
    );
...
async function getSearchResults(search) {
    ...
    enviarHistorialAlStorage().then(crearBotonesDeHistorial);
...
function ponerFrases() {
    ActivarBotonBuscar();
    let desplegableSugerencias = document.getElementById(
...
function cargarPagina() {
    cargarHistorialDelStorage().then(crearBotonesDeHistorial);
    getTrendingResults();
    colocarRandoms();
